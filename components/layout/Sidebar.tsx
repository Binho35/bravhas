...
export function Sidebar() {
  ...
  const visibleSections = sections
    .map((section) => ({ ...section, items: section.items.filter((item) => item.allowed) }))
  ...
  return (
    <div className="flex h-full flex-col bg-[#0B2947] text-white">
      <div className="border-b border-white/10 px-6 py-6 pr-16 lg:pr-6">
        <h1 className="text-2xl font-bold tracking-tight">Brav<span className="text-[#8CC4EA]">HAS</span></h1>
      ...
      <nav aria-label="Navegação principal" className="flex-1 overflow-auto px-3 py-4">
        {visibleSections.map((section) => (
          <div key={section.title} className="mb-6">
            <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.18em] text-white/45">{section.title}</p>
              ...
              {section.items.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex min-h-11 items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70 ${active ? "bg-[#154B7A] text-white shadow-sm" : "text-white/75 hover:bg-white/5 hover:text-white"}`}
                  >
