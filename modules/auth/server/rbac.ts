...
export const RBAC_RESOURCES = ["colaboradores","admissoes","recrutamento","desempenho","canal-rh","ponto","ferias","beneficios","afastamentos","medidas-disciplinares","desligamentos","folha","organizacao","relatorios","auditoria","configuracoes"] as const;
export type RbacResource = (typeof RBAC_RESOURCES)[number];
export type RbacAction = "view" | "create" | "edit" | "approve" | "delete" | "export";

type PermissionRow = { master: boolean; canView: boolean; canCreate: boolean; canEdit: boolean; canApprove: boolean; canDelete: boolean; canExport: boolean };

const actionColumn: Record<RbacAction, keyof PermissionRow> = { view:"canView", create:"canCreate", edit:"canEdit", approve:"canApprove", delete:"canDelete", export:"canExport" };

export async function requirePermission(resource: RbacResource, action: RbacAction) {
  if (process.env.NODE_ENV === "development" && process.env.BRAVHAS_DEV_AUTH_BYPASS !== "false") return null;
...
export async function ensureDefaultAccessProfiles(companyId: string) {
  ...
  for (const profile of profiles) {
    await prisma.$executeRawUnsafe(`INSERT INTO "AccessProfile" ("id","companyId","name","description","master","system","active","updatedAt") VALUES ($1,$2,$3,$4,$5,true,true,NOW()) ON CONFLICT ("companyId","name") DO NOTHING`, randomUUID(), companyId, profile.name, profile.description, profile.master);
  }
}
