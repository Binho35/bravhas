...
export class RegisterReceiptUseCase {
  ...
  async execute(input: RegisterReceiptInput): Promise<RegisterReceiptResult> {
    const accountId = input.accountId.trim();
    if (!accountId) throw new Error("A conta financeira é obrigatória.");

    const receivedBy = input.receivedBy.trim();
    if (!receivedBy) throw new Error("O responsável pelo recebimento é obrigatório.");
    ...
    const currentReceived = roundCurrency(account.data.paidAmount);
    ...
    if (existing) {
      const fullyReceived = currentReceived >= total - 0.001;
      return {
        account,
        transactionId: existing.id,
        fullyReceived,
        remaining: fullyReceived ? 0 : Math.max(0, roundCurrency(total - currentReceived)),
      };
    }

    if (account.data.status === "PAID" || account.data.status === "CANCELED") {
      throw new Error("Esta conta não permite um novo recebimento.");
    }

    const remainingBeforeReceipt = roundCurrency(total - currentReceived);
    if (remainingBeforeReceipt <= 0) throw new Error("Esta conta não possui saldo pendente.");
    if (normalizedAmount > remainingBeforeReceipt + 0.001) {
      throw new Error("O valor do recebimento é maior que o saldo restante.");
    }

    const nextReceivedAmount = roundCurrency(currentReceived + normalizedAmount);
    const fullyReceived = nextReceivedAmount >= total - 0.001;
    const receiptDate = input.receiptDate ?? new Date();
    const updatedAccount = account.update({
      paidAmount: fullyReceived ? total : nextReceivedAmount,
      paymentDate: receiptDate,
      status: fullyReceived ? "PAID" : "PARTIALLY_PAID",
    ...
    const transaction = await this.transactionRepository.create({
      ...
      performedAt: receiptDate.toISOString(),
      notes: fullyReceived ? "Recebimento integral registrado." : "Recebimento parcial registrado.",
    });
    ...
    return {
      ...
      fullyReceived,
      remaining: fullyReceived ? 0 : roundCurrency(total - nextReceivedAmount),
    };
  }
}
