...
import type { FinancialAccountStatus, FinancialAccountType } from "../domain/entities/FinancialAccount";
...
function getAccountTotal(account: CashFlowAccount): number {
  return roundCurrency(account.amount - account.discount + account.interest + account.fine);
}

function getAccountRemaining(account: CashFlowAccount): number {
  return Math.max(0, roundCurrency(getAccountTotal(account) - account.paidAmount));
}

function isActiveAccount(account: CashFlowAccount): boolean {
  return account.status !== "PAID" && account.status !== "CANCELED";
}
...
export function calculateCashFlow({ accounts, transactions, openingBalance = 0, startDate, endDate }: CalculateCashFlowInput): CashFlowSummary {
  ...
  if (effectiveEnd.getTime() < effectiveStart.getTime()) {
    throw new Error("A data final do fluxo de caixa não pode ser anterior à data inicial.");
  }
  ...
  const ensureBucket = (dateKey: string) => {
    if (!bucketMap.has(dateKey)) bucketMap.set(dateKey, { inflow: 0, outflow: 0 });
  ...
  for (const account of activeAccounts) {
    ...
    if (account.type === "RECEIVABLE") bucket.inflow = roundCurrency(bucket.inflow + remaining);
    else bucket.outflow = roundCurrency(bucket.outflow + remaining);
  }

  const buckets: CashFlowBucket[] = [];
  let runningBalance = roundCurrency(openingBalance);
  for (let date = new Date(effectiveStart); date.getTime() <= effectiveEnd.getTime(); date.setDate(date.getDate() + 1)) {
    const dateKey = toDateKey(date);
    const bucket = bucketMap.get(dateKey) ?? { inflow: 0, outflow: 0 };
    const net = roundCurrency(bucket.inflow - bucket.outflow);
    runningBalance = roundCurrency(runningBalance + net);
    buckets.push({ date: dateKey, inflow: bucket.inflow, outflow: bucket.outflow, net, projectedBalance: runningBalance });
  }

  return { totalReceivable, totalPayable, totalReceived, totalPaid, projectedBalance, overdueReceivable, overduePayable, buckets };
}
