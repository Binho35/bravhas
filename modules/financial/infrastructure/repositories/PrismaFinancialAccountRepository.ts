...
type FinancialAccountDbClient = Pick<typeof prisma, "financialAccount">;

export class PrismaFinancialAccountRepository implements FinancialAccountRepository {
  constructor(private readonly db: FinancialAccountDbClient = prisma) {}

  async create(account: FinancialAccount): Promise<FinancialAccount> {
    const data = account.data;
    const created = await this.db.financialAccount.create({
      data: {
        id: data.id,
  ...
  async findById(id: string): Promise<FinancialAccount | null> {
    const account = await this.db.financialAccount.findUnique({ where: { id } });
    return account ? this.toDomain(account) : null;
  }

  async findAll(filters: FindFinancialAccountsFilters): Promise<FinancialAccount[]> {
  ...
  async update(account: FinancialAccount): Promise<FinancialAccount> {
    const data = account.data;
    const updated = await this.db.financialAccount.update({
      where: { id: data.id },
      data: {
        companyId: data.companyId,
        branchId: data.branchId,
  ...
  async delete(id: string): Promise<void> {
    await this.db.financialAccount.delete({ where: { id } });
  }
  ...
  private toDomain(account: {
    ...
    return new FinancialAccount({
      ...
      status: account.status as FinancialAccountStatus,
