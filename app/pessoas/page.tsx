...
async function loadDashboardMetrics() {
  try {
    ...
    if (!company) return { active: 0, admissions: 0, point: 0, vacations: 0, tickets: 0 };
    ...
    const [active, admissions, point, vacations, tickets] = await Promise.all([
      ...
      prisma.hrVacationRequest.count({ where: { companyId: company.id, startDate: { gte: now, lte: in30Days }, status: { in: ["PENDING", "APPROVED"] } } }),
    ...
    return { active: 0, admissions: 0, point: 0, vacations: 0, tickets: 0 };
  }
}
        ...
        <div className="mx-auto max-w-7xl">
          <section className="overflow-hidden rounded-[32px] bg-gradient-to-br from-[#0b2947] via-[#103d65] to-[#154b7a] p-6 text-white shadow-[0_24px_80px_-32px_rgba(11,41,71,0.65)] md:p-8">
            <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-3xl">
                <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-semibold text-blue-100"><Sparkles className="h-3.5 w-3.5" />Gestão integrada de pessoas</div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:flex"><Link href="/rh" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-4 py-3 text-sm font-bold text-[#0b2947] shadow-lg shadow-black/10 transition hover:-translate-y-0.5"><HeartHandshake className="h-4 w-4" /> RH</Link><Link href="/dp" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/20 bg-white/10 px-4 py-3 text-sm font-bold text-white transition hover:bg-white/15"><BadgeCheck className="h-4 w-4" /> DP</Link></div>
            </div>
          </section>

          <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {metrics.map((metric) => { const Icon = metric.icon; return <article key={metric.label} className="rounded-[26px] border border-slate-200/80 bg-white p-5 shadow-[0_12px_35px_-24px_rgba(15,23,42,0.35)]"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-medium text-slate-500">{metric.label}</p><p className="mt-2 text-3xl font-bold tracking-tight text-slate-950">{metric.value}</p></div><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#eaf3fb] text-[#154b7a]"><Icon className="h-5 w-5" /></div></div><p className="mt-4 text-xs font-medium text-slate-400">{metric.helper}</p></article>; })}
          </section>
          ...
          <section className="mt-6 grid gap-6 xl:grid-cols-[1.45fr_0.75fr]">
            <article className="rounded-[30px] border border-slate-200/80 bg-white p-6 shadow-[0_16px_45px_-30px_rgba(15,23,42,0.3)]">
              <div className="flex items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#154b7a]">Prioridades</p><h2 className="mt-1 text-xl font-bold tracking-tight text-slate-950">O que precisa de atenção</h2></div><FileWarning className="h-5 w-5 text-amber-500" /></div>
              <div className="mt-5 divide-y divide-slate-100">
                {priorities.map((item) => <Link key={item.href} href={item.href} className="group flex items-center gap-4 py-4 first:pt-0 last:pb-0"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-slate-100 text-xs font-bold text-slate-600 group-hover:bg-[#eaf3fb] group-hover:text-[#154b7a]">{item.tag}</div><div className="min-w-0 flex-1"><p className="font-semibold text-slate-900">{item.title}</p><p className="mt-1 text-sm leading-5 text-slate-500">{item.description}</p></div><ArrowUpRight className="h-4 w-4 shrink-0 text-slate-300 transition group-hover:text-[#154b7a]" /></Link>)}
              </div>
            </article>
