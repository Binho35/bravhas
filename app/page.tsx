  ...
  return (
    <AppShell sidebar={<Sidebar />} header={<Header />}>
      <div className="h-full overflow-auto p-5">
        ...
        {loading && <div className="mb-4 rounded-xl border border-[#E2E8F0] bg-white p-4 text-sm text-[#64748B]">Carregando indicadores...</div>}
        {error && <div className="mb-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-[#DC2626]">{error}</div>}

        <section className="grid grid-cols-4 gap-3">
          <div className="rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-sm"><p className="text-xs text-[#64748B]">Contas a pagar</p><p className="mt-2 text-xl font-bold text-[#DC2626]">{formatCurrency(totalPayable)}</p><p className="mt-1 text-[10px] text-[#94A3B8]">{payableAccounts.length} em aberto</p></div>
          <div className="rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-sm"><p className="text-xs text-[#64748B]">Contas a receber</p><p className="mt-2 text-xl font-bold text-[#16A34A]">{formatCurrency(totalReceivable)}</p><p className="mt-1 text-[10px] text-[#94A3B8]">{receivableAccounts.length} em aberto</p></div>
        ...
        <section className="mt-4 grid grid-cols-[1.3fr_0.7fr] gap-4">
          <div className="rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-sm">
            <h3 className="text-sm font-bold text-[#0B2947]">Atenções prioritárias</h3>
            <div className="mt-3 space-y-2">
              {attentionItems.slice(0, 8).map((item) => (
                <div key={item.id} className="flex items-center justify-between rounded-xl border border-[#F1F5F9] px-3 py-2.5">
                  <div><p className="text-sm font-semibold text-[#0F172A]">{item.title}</p><p className="text-[11px] text-[#94A3B8]">{areaLabels[item.area]} • {item.responsibleName}</p></div>
                  <div className="text-right"><p className="text-xs font-bold text-[#D97706]">{item.priority === "CRITICAL" ? "Crítica" : "Alta"}</p><p className="text-[10px] text-[#94A3B8]">{formatDeadline(item.dueDate, referenceDate)}</p></div>
                </div>
              ))}
          ...
          <div className="rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 space-y-4">
              {monitoredAreas.map((item) => {
                ...
                return <div key={item.area}><div className="flex items-center justify-between"><span className="text-xs font-semibold text-[#475569]">{item.name}</span><span className="text-xs font-bold text-[#154B7A]">{health}%</span></div><div className="mt-1 h-2 rounded-full bg-[#EAF3FB]"><div className="h-2 rounded-full bg-[#154B7A]" style={{ width: `${health}%` }} /></div></div>;
              })}
            </div>
          </div>
        </section>
