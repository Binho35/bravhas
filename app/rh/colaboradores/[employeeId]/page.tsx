...
export default async function EmployeeDossierPage({
  ...
  const timeline = [
    ...employee.documents.map((item) => ({
      ...
      type: "Documento",
      title: item.title,
      detail: item.verifiedAt ? "Documento conferido" : "Aguardando conferência",
    })),
    ...employee.timeOccurrences.map((item) => ({
      id: `point-${item.id}`,
      date: item.createdAt,
      type: "Ponto",
      title: item.type,
    ...
    ...employee.vacationRequests.map((item) => ({
      id: `vacation-${item.id}`,
      date: item.createdAt,
      type: "Férias",
      title: `${dateLabel(item.startDate)} a ${dateLabel(item.endDate)}`,
      detail: item.status,
    })),
    ...employee.leaveRequests.map((item) => ({
      id: `leave-${item.id}`,
      date: item.createdAt,
      type: "Afastamento",
      title: item.type,
      detail: `${dateLabel(item.startDate)} a ${dateLabel(item.endDate)}`,
    })),
  ]
    ...
    <main className="px-4 py-6 text-slate-950 md:px-7 md:py-8">
      <div className="mx-auto max-w-[1360px]">
        <Link
          ...
          className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 transition hover:text-[#154b7a]"
        >
          ...
          <div className="bg-gradient-to-r from-[#0b2947] via-[#103d65] to-[#154b7a] px-6 py-7 text-white md:px-8">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="flex items-start gap-4">
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[22px] bg-white/10 ring-1 ring-white/15">
                  <UserRound className="h-7 w-7" />
                </div>
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-100">Dossiê funcional</p>
                    <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ring-1 ${statusClass[employee.status] ?? statusClass.PRE_ADMISSION}`}>
                  ...
                  <p className="mt-2 text-sm text-blue-100">
                    {employee.position?.name ?? "Cargo não definido"} · {employee.department?.name ?? "Departamento não definido"}
                  </p>
                </div>
              </div>
        ...
        <section className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ["Documentos", employee._count.documents, FileText],
            ["Ocorrências de ponto", employee._count.timeOccurrences, BadgeCheck],
            ["Férias", employee._count.vacationRequests, CalendarDays],
            ["Afastamentos", employee._count.leaveRequests, HeartPulse],
          ].map(([label, value, Icon]) => {
            const Component = Icon as typeof FileText;
            return (
              <article key={String(label)} className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-[0_8px_30px_rgba(15,23,42,0.04)]">
                <div className="flex items-start justify-between gap-4">
                  <div><p className="text-xs font-medium text-slate-500">{label}</p><strong className="mt-2 block text-2xl text-[#0b2947]">{String(value)}</strong></div>
                  <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#eaf3fb] text-[#154b7a]"><Component className="h-[18px] w-[18px]" /></div>
                </div>
              </article>
            );
          })}
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[1fr_1fr]">
          <article className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-[0_8px_30px_rgba(15,23,42,0.04)]">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
              <BriefcaseBusiness className="h-5 w-5 text-[#154b7a]" />
              <div><h2 className="font-bold text-[#0b2947]">Vínculo e contrato</h2><p className="text-xs text-slate-500">Dados funcionais consolidados.</p></div>
            </div>
            <dl className="mt-5 grid gap-5 sm:grid-cols-2">
              {[
                ...
                <div key={label}><dt className="text-[11px] font-medium text-slate-400">{label}</dt><dd className="mt-1 text-sm font-semibold text-slate-700">{value}</dd></div>
              ))}
            </dl>
          </article>
          ...
          <article className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-[0_8px_30px_rgba(15,23,42,0.04)]">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
              <ShieldCheck className="h-5 w-5 text-[#154b7a]" />
              <div><h2 className="font-bold text-[#0b2947]">Governança do dossiê</h2><p className="text-xs text-slate-500">Cobertura dos processos vinculados.</p></div>
            </div>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {[
                ...
                <div key={String(label)} className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-4">
                  <p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-xl font-bold text-[#0b2947]">{String(value)}</p>
                </div>
              ))}
            </div>
          </article>
        ...
        <section className="mt-5 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-[0_8px_30px_rgba(15,23,42,0.04)]">
          <div className="flex items-center justify-between gap-4 border-b border-slate-100 pb-4">
            <div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#154b7a]">Histórico funcional</p><h2 className="mt-1 text-lg font-bold text-[#0b2947]">Linha do tempo</h2></div>
            <span className="text-xs text-slate-400">Eventos mais recentes</span>
          </div>
          ...
          {timeline.length === 0 ? (
            ...
            <div className="divide-y divide-slate-100">
              {timeline.map((item) => (
                <div key={item.id} className="grid gap-2 py-4 md:grid-cols-[120px_130px_1fr] md:items-start">
                  <span className="text-xs font-medium text-slate-400">{dateLabel(item.date)}</span>
                  <span className="w-fit rounded-full bg-[#eaf3fb] px-2.5 py-1 text-[11px] font-semibold text-[#154b7a]">{item.type}</span>
                  <div><p className="text-sm font-semibold text-slate-800">{item.title}</p><p className="mt-1 text-sm text-slate-500">{item.detail}</p></div>
                </div>
              ))}
            </div>
          )}
