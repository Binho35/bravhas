...
export default async function EmployeesPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  return <main className="px-4 py-6 text-slate-950 md:px-7 md:py-8"><div className="mx-auto max-w-[1360px]">
    <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#154b7a]">People Core</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-[#0b2947]">Colaboradores</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Cadastro mestre de pessoas, vínculo, unidade, setor, cargo, gestor, documentos e histórico funcional.</p></div><Link href="/rh/colaboradores/novo" className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-[#0b2947] px-5 text-sm font-semibold text-white shadow-lg shadow-blue-950/10 transition hover:bg-[#154b7a]"><UserPlus className="h-4 w-4" />Novo colaborador</Link></div>
    <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{summary.map(([label,value,detail],index)=><article key={label} className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-[0_8px_30px_rgba(15,23,42,0.05)]"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-medium text-slate-500">{label}</p><strong className="mt-2 block text-2xl text-[#0b2947]">{value}</strong></div><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#eaf3fb] text-[#154b7a]">{index===0?<UsersRound className="h-[18px] w-[18px]"/>:index===1?<UserPlus className="h-[18px] w-[18px]"/>:index===2?<BadgeCheck className="h-[18px] w-[18px]"/>:<FileText className="h-[18px] w-[18px]"/>}</div></div><p className="mt-4 text-[11px] text-slate-400">{detail}</p></article>)}</section>
    <section className="mt-5 overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.05)]"><div className="flex flex-col gap-4 border-b border-slate-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#154b7a]">Base corporativa</p><h2 className="mt-1 text-lg font-bold">Todos os colaboradores</h2></div><form method="get" className="flex w-full flex-col gap-2 sm:flex-row lg:w-auto"><label className="relative min-w-0 flex-1 lg:w-80"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"/><input name="q" defaultValue={q} placeholder="Nome, CPF, matrícula, cargo ou setor" className="h-11 w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-4 text-sm outline-none focus:border-[#154b7a]"/></label><select name="status" defaultValue={status} className="h-11 rounded-2xl border border-slate-200 bg-white px-3 text-sm text-slate-600"><option value="">Todos os status</option><option value="ACTIVE">Ativos</option><option value="PRE_ADMISSION">Pré-admissão</option><option value="ON_LEAVE">Afastados</option><option value="TERMINATED">Desligados</option></select><button className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-[#154b7a] px-4 text-sm font-semibold text-white"><Filter className="h-4 w-4"/>Aplicar</button>{q||status?<Link href="/rh/colaboradores" className="inline-flex h-11 items-center justify-center rounded-2xl border border-slate-200 px-4 text-sm font-semibold text-slate-600">Limpar</Link>:null}</form></div>
