...
export default async function VacationsPage() {
  ...
  const [employees, requests, pending, approved, upcoming] = companyId
    ? await Promise.all([
        ...
        prisma.hrVacationRequest.findMany({ where: { companyId }, orderBy: [{ startDate: "asc" }, { createdAt: "desc" }], take: 50, include: { employee: { select: { id: true, fullName: true, employeeNumber: true } } } }),
    ...
    <main className="px-4 py-6 text-slate-950 md:px-7 md:py-8">
      <div className="mx-auto max-w-[1360px]">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#154b7a]">DP · Férias</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-[#0b2947]">Férias</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Programação, aprovação, abono, adiantamento de 13º e histórico de férias vinculados ao dossiê funcional.</p></div>
          <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-xs font-semibold text-[#154b7a]">Gestão centralizada · histórico automático</div>
        </div>

        <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {metrics.map(({ label, value, icon: Icon }) => <article key={label} className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-[0_8px_30px_rgba(15,23,42,0.05)]"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-medium text-slate-500">{label}</p><strong className="mt-2 block text-2xl text-[#0b2947]">{value}</strong></div><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#eaf3fb] text-[#154b7a]"><Icon className="h-[18px] w-[18px]" /></div></div></article>)}
        </section>
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[430px_1fr]">
          <form action={createVacation} className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-[0_8px_30px_rgba(15,23,42,0.05)]">
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="text-xs font-semibold text-slate-600">Início *</span><input name="startDate" type="date" required className="mt-2 h-11 w-full rounded-2xl border border-slate-200 px-3 text-sm outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-50" /></label><label className="block"><span className="text-xs font-semibold text-slate-600">Fim *</span><input name="endDate" type="date" required className="mt-2 h-11 w-full rounded-2xl border border-slate-200 px-3 text-sm outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-50" /></label></div>
              <label className="block"><span className="text-xs font-semibold text-slate-600">Dias de abono</span><input name="sellDays" type="number" min="0" max="10" defaultValue="0" className="mt-2 h-11 w-full rounded-2xl border border-slate-200 px-3 text-sm outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-50" /></label>
              ...
              <label className="block"><span className="text-xs font-semibold text-slate-600">Observações</span><textarea name="notes" rows={3} placeholder="Informações relevantes para análise." className="mt-2 w-full rounded-2xl border border-slate-200 p-3 text-sm outline-none placeholder:text-slate-300 focus:border-blue-300 focus:ring-4 focus:ring-blue-50" /></label>
              <button type="submit" className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-[#0b2947] px-4 text-sm font-semibold text-white shadow-lg shadow-blue-950/10 transition hover:bg-[#154b7a]"><CalendarDays className="h-4 w-4" />Registrar programação</button>
            </div>
          </form>
          ...
          <article className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.05)]">
            ...
            {requests.length === 0 ? <div className="px-6 py-14 text-center"><Palmtree className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-4 font-semibold text-slate-700">Nenhuma programação registrada</p><p className="mt-2 text-sm text-slate-500">As férias programadas aparecerão aqui para acompanhamento.</p></div> : <div className="divide-y divide-slate-100">{requests.map((item) => <div key={item.id} className="grid gap-3 px-6 py-4 md:grid-cols-[minmax(180px,1fr)_180px_120px_130px] md:items-center"><div><p className="text-sm font-semibold text-slate-800">{item.employee.fullName}</p><p className="mt-1 text-xs text-slate-400">{item.employee.employeeNumber ?? "Sem matrícula"}</p></div><div><p className="text-xs font-medium text-slate-500">{new Intl.DateTimeFormat("pt-BR").format(item.startDate)} → {new Intl.DateTimeFormat("pt-BR").format(item.endDate)}</p><p className="mt-1 text-xs text-slate-400">Abono: {item.sellDays} dia(s)</p></div><span className="text-xs text-slate-500">13º: {item.advance13th ? "Sim" : "Não"}</span><span className="w-fit rounded-full bg-blue-50 px-2.5 py-1 text-[11px] font-semibold text-blue-700 ring-1 ring-blue-100">{statusLabel[item.status] ?? item.status}</span></div>)}</div>}
          </article>
        </section>
      </div>
    </main>
