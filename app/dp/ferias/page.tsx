...
async function reviewVacation(formData: FormData) {
  ...
  if (!current) throw new Error("Solicitação não encontrada ou fora do escopo autorizado.");
...
export default async function VacationsPage() {
  ...
  const [requests, pending, approved, upcoming] = visibleEmployeeIds.length > 0 ? await Promise.all([
    prisma.hrVacationRequest.findMany({ where: { companyId: employeeScope.companyId, employeeId: { in: visibleEmployeeIds } }, orderBy: [{ startDate: "asc" }, { createdAt: "desc" }], take: 80, include: { employee: { select: { id: true, fullName: true, employeeNumber: true } } } }),
    prisma.hrVacationRequest.count({ where: { companyId: employeeScope.companyId, employeeId: { in: visibleEmployeeIds }, status: "PENDING" } }),
    prisma.hrVacationRequest.count({ where: { companyId: employeeScope.companyId, employeeId: { in: visibleEmployeeIds }, status: "APPROVED" } }),
    prisma.hrVacationRequest.count({ where: { companyId: employeeScope.companyId, employeeId: { in: visibleEmployeeIds }, status: "APPROVED", startDate: { gte: now, lte: in30Days } } }),
  ]) : [[], 0, 0, 0];
  ...
  const metrics = [
    ...
    { label: "Base autorizada", value: employees.length, icon: Palmtree },
  ];
    ...
    <main className="px-4 py-6 text-slate-950 md:px-7 md:py-8">
      <div className="mx-auto max-w-[1360px]">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          ...
          <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-xs font-semibold text-[#154b7a]">Gestão centralizada · aprovação rastreável</div>
        </div>

        <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Indicadores de férias">
