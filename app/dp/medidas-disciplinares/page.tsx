...
async function createAction(formData: FormData) {
  ...
  const employee = await prisma.hrEmployee.findFirst({
    where: employeeEligibilityByIdWhere(actor.companyId, employeeId),
    select: { id: true },
  });
  if (!employee) throw new Error("Colaborador ainda não está elegível para medidas disciplinares.");
...
export default async function DisciplinaryPage() {
  ...
  const [employees, actions] = await Promise.all([
    prisma.hrEmployee.findMany({ where: employeeEligibilityWhere(companyId), orderBy: { fullName: "asc" }, select: { id: true, fullName: true } }),
