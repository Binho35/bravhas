...
import { employeeEligibilityByIdWhere, employeeEligibilityStateWhere } from "@/modules/hrdp/workflows/employeeEligibility";
...
async function requireOperationalEmployee(companyId: string, employeeId: string) {
  const employee = await prisma.hrEmployee.findFirst({
    where: employeeEligibilityByIdWhere(companyId, employeeId),
    select: { id: true },
  });
  if (!employee) throw new Error("Colaborador ainda não está elegível para rotinas de ponto.");
}
...
async function createOccurrence(formData: FormData) {
  ...
  await requireOperationalEmployee(actor.companyId, employeeId);
...
export default async function AttendancePage() {
  ...
  const employees = await prisma.hrEmployee.findMany({
    where: { ...employeeScope, ...employeeEligibilityStateWhere() },
      ...
      <div className="mx-auto max-w-[1360px]">
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[430px_1fr]">
          <form action={createOccurrence} className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-[0_8px_30px_rgba(15,23,42,0.05)] sm:p-6">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-4"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#eaf3fb] text-[#154b7a]" aria-hidden="true"><Plus className="h-5 w-5" /></div><div><h2 className="font-bold text-[#0b2947]">Nova ocorrência</h2><p className="text-xs text-slate-500">Registro inicial para tratamento do gestor e conferência do DP.</p></div></div>
