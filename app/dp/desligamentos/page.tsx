...
import { employeeEligibilityByIdWhere, employeeEligibilityWhere } from "@/modules/hrdp/workflows/employeeEligibility";
...
async function terminateEmployee(formData: FormData) {
  "use server";
  const actor = await hrdpPermission.desligamentos("approve");
  ...
  const employee = await prisma.hrEmployee.findFirst({ where: employeeEligibilityByIdWhere(actor.companyId, employeeId), select: { id: true } });
  if (!employee) throw new Error("Colaborador ainda não está elegível para desligamento.");
  const reason = text(formData, "reason");
  const date = new Date(`${terminationDate}T12:00:00`);
  await prisma.$transaction([
    prisma.hrEmployee.update({ where: { id: employeeId }, data: { status: "TERMINATED", active: false, terminationDate: date, notes: reason ? `Desligamento: ${reason}` : undefined } }),
    prisma.hrBenefitEnrollment.updateMany({ where: { companyId: actor.companyId, employeeId, active: true }, data: { active: false, endedAt: date } }),
    prisma.hrVacationRequest.updateMany({ where: { companyId: actor.companyId, employeeId, status: { in: ["DRAFT", "PENDING", "APPROVED"] } }, data: { status: "CANCELED" } }),
  ]);
  await logHrdpAudit({ companyId: actor.companyId, actorUserId: actor.id, action: "EMPLOYEE_TERMINATED", entityType: "HrEmployee", entityId: employeeId, metadata: { terminationDate, reason: reason ?? "" } });
  revalidatePath("/dp/desligamentos"); revalidatePath("/rh/colaboradores"); revalidatePath("/pessoas"); revalidatePath(`/rh/colaboradores/${employeeId}`);
}

export default async function TerminationsPage(){
  const actor=await hrdpPermission.desligamentos("view"); const companyId=actor.companyId;
  const [activeEmployees,terminated]=await Promise.all([
    prisma.hrEmployee.findMany({where:employeeEligibilityWhere(companyId),orderBy:{fullName:"asc"},select:{id:true,fullName:true,employeeNumber:true}}),
  ...
  <section className="mt-5 grid gap-5 xl:grid-cols-[420px_1fr]"><form action={terminateEmployee} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-rose-50 text-rose-700"><UserMinus className="h-5 w-5"/></div><div><h2 className="font-bold text-[#0b2947]">Encerrar vínculo</h2><p className="text-xs text-slate-500">Somente colaboradores admitidos e operacionalmente elegíveis podem ser desligados.</p></div></div><div className="mt-5 space-y-4"><select name="employeeId" required className="h-11 w-full rounded-2xl border border-slate-200 px-3 text-sm"><option value="">Selecione o colaborador</option>{activeEmployees.map(i=><option key={i.id} value={i.id}>{i.fullName}{i.employeeNumber?` · ${i.employeeNumber}`:""}</option>)}</select><input name="terminationDate" required type="date" className="h-11 w-full rounded-2xl border border-slate-200 px-3 text-sm"/><select name="reason" className="h-11 w-full rounded-2xl border border-slate-200 px-3 text-sm"><option value="">Motivo</option><option>Pedido de demissão</option><option>Dispensa sem justa causa</option><option>Dispensa por justa causa</option><option>Término de contrato</option><option>Acordo</option><option>Outro</option></select><button className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-rose-700 text-sm font-semibold text-white"><LogOut className="h-4 w-4"/>Concluir desligamento</button><p className="text-xs leading-5 text-slate-400">A conclusão encerra os benefícios ativos, cancela programações pendentes de férias e preserva o dossiê para consulta histórica.</p></div></form>
