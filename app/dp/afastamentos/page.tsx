...
async function createLeave(formData: FormData) {
  ...
  await prisma.$transaction([
    ...
    prisma.hrEmployee.update({ where: { id: employeeId }, data: { status: "ON_LEAVE" } }),
  ]);
...
async function reviewLeave(formData: FormData) {
  ...
  if (!id || !employeeId || !decision) throw new Error("Afastamento inválido.");
  if (!["APPROVED", "REJECTED", "COMPLETED", "CANCELED"].includes(decision)) throw new Error("Status inválido.");

  const leave = await prisma.hrLeaveRequest.findUnique({ where: { id }, select: { endDate: true } });
  if (!leave) throw new Error("Afastamento não encontrado.");
