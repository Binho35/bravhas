      ...
      <AppShell sidebar={<Sidebar />} header={<Header />}>
        <OperationalModulePage eyebrow="Gestão" title="Documentos" description="Central tenant-safe dos documentos reais vinculados aos colaboradores." statusText="Fonte real · RH/DP">
          ...
          <section className="overflow-hidden rounded-2xl border border-[#E2E8F0] bg-white shadow-sm">
            ...
            {loading ? <div className="p-8 text-sm text-[#64748B]">Carregando documentos...</div> : filtered.length === 0 ? <div className="p-10 text-center"><FileText className="mx-auto text-[#94A3B8]" size={30} /><p className="mt-3 text-sm font-semibold text-[#475569]">{documents.length ? "Nenhum documento corresponde à busca." : "Nenhum documento cadastrado."}</p></div> : <div className="divide-y divide-[#F1F5F9]">{filtered.map((item) => <div key={item.id} className="grid gap-3 px-5 py-4 md:grid-cols-[1fr_180px_130px_190px] md:items-center"><div><Link href={`/documentos/${item.id}`} className="text-sm font-semibold text-[#0F172A] hover:text-[#154B7A] hover:underline">{item.title}</Link><p className="mt-1 text-xs text-[#64748B]">{item.employee.fullName} · {item.type}</p></div><div className="text-xs text-[#64748B]">Validade: {date(item.expiresAt)}</div><div className={`text-xs font-semibold ${item.verifiedAt ? "text-emerald-700" : "text-amber-700"}`}>{item.verifiedAt ? `Conferido ${date(item.verifiedAt)}` : "Pendente"}</div><div className="flex justify-end gap-3"><Link href={`/documentos/${item.id}`} className="text-xs font-semibold text-[#154B7A]">Metadados</Link>{item.fileAvailable ? <Link href={`/api/hr/documents/${item.id}/file`} target="_blank" className="text-xs font-semibold text-[#154B7A]">Arquivo</Link> : <Link href={`/rh/colaboradores/${item.employee.id}/documentos`} className="text-xs font-semibold text-[#154B7A]">Dossiê</Link>}</div></div>)}</div>}
          </section>
